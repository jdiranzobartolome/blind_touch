...
class Mp3player:
    ...
    def __init__(self, audios_path, output_device, input_device = '0'):
        ...
        #From the following two commands, only one can be uncommented. One starts pulseaudio in the terminal
        ...
        error = True
        while (error != False):
            response = subprocess.check_output(["pacmd", "set-default-sink", str(output_device)])
            error = 'does not exist' in response.decode('utf-8')
            if ((DEBUG==1) and (error == True)):
                print('output device string not recognized')
            elif ((DEBUG==1) and (error == False)):
                print('output device string correctly recognized')
        if (input_device != '0'):
            error = True
            while (error != False):
                response = subprocess.check_output(["pacmd", "set-default-source", str(input_device)])
                error = 'does not exist' in response.decode('utf-8')
                if ((DEBUG==1) and (error == True)):
                    print('input device string not recognized')
                elif ((DEBUG==1) and (error ==False)):
                    print('input device correctly recognized')
    ...
    def play_audio(self, rel_path, volume = OUTPUT_VOLUME):
        ...
        if (DEBUG==0):
            ...
            subprocess.Popen(["/usr/bin/mpg321", filename + ".mp3","2>&1"])
        #os.system('mpg321 ' + filename + '.mp3')
        return 1    
