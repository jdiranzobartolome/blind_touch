...
class MicThread (threading.Thread):
   ...
   def __init__(self, name, threadLock, q_voice, mp3player, google_voice, paint_mode, language_code):
   ...
   def run(self):
      ...
      mic_thread_main(self.threadLock, self.q_voice, self.mp3player, self.google_voice, self.paint_mode, self.language_code)
      print ("Exiting " + str(self.name))

def stop_till_on(q_voice, keyword_bool, sentence):
    #threading.Event obejct has three main calls: set, clear and wait. (and the boolean is_set() 
    #set: sets a flag to 1.
    ...
    if (command == "off"):
        ...
        while (command == "off"):
            time.sleep(1)
            if (not q_voice.empty()):
                command = q_voice.get()
        if (DEBUG == 1):
            print("Mic on")
        return keyword_bool, sentence
    else:
        return keyword_bool, sentence
...
#
def mic_thread_main(threadLock, q_voice, mp3player, google_voice, paint_mode, language_code):
   ...
   while True:
        ...
        if (VOICE_FEEDBACK == 1):
         ...
         ## Here we...
        keyword_bool, sentence = stop_till_on(q_voice, keyword_bool, sentence)
