...
def main():
    ...
    #Defining variables    
    # Create new threads, shareable threadLock and queues.
    #only voice thread and tapping thread will share threadlocks since those are the only ones that have to sincronize activities
    ...
    voice_thread = MicThread("voice-thread",threadLock, q_voice, mp3player, google_voice, paint_mode, language_code)
    if (CONTROL_API == 1):
        control_thread = ControlThread("control-thread", q_voice)
    ...
    while (len(threads) > 0):
        try:
            threads = [t.join(50) for t in threads if t is not None and t.isAlive()]
